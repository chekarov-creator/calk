...
export class ExportService {
  ...
  exportPDF() {
    ...
    const head = [['Тип', 'ГОСТ', 'Типоразмер', 'S (см²)', 'P (см)', 'ПТМ (мм)', 'Предел огнест.', 'Пл/м (м²)', 'Пл/т (м²)', 'Толщ (мм)', 'Расх (кг/м²)', 'Кол-во', 'Ед', 'Итого (кг)']];
    const body = rows.map(r => [r['Тип'], r['ГОСТ'], r['Типоразмер'], r['S (см²)'], r['P (см)'], r['ПТМ (мм)'], r['Предел огнестойкости'], r['Площадь/м (м²)'], r['Площадь/т (м²)'], r['Толщина (мм)'], r['Расход (кг/м²)'], r['Количество'], r['Ед'], r['Итого (кг)']]);
