...
export class ExportService {
  ...
  exportPDF() {
    ...
    const head = [['Тип', 'ГОСТ', 'Размер', 'Периметр обогреваемой поверхности см', 'Площадь сечения см²', 'ПТМ (мм)', 'Предел огнест.', 'Площадь мп/м²', 'Площадь тн/м²', 'Толщина нанесения', 'Расх (кг/м²)', 'Кол-во', 'Ед', 'Итого (кг)']];
    const body = rows.map(r => [r['Тип'], r['ГОСТ'], r['Размер'], r['Периметр обогреваемой поверхности см'], r['Площадь сечения см²'], r['ПТМ (мм)'], r['Предел огнестойкости'], r['Площадь мп/м²'], r['Площадь тн/м²'], r['Толщина нанесения'], r['Расход (кг/м²)'], r['Количество'], r['Ед'], r['Итого (кг)']]);
